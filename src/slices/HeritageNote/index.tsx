...
const HeritageNote: FC<Props> = ({ slice }) => {
  ...
  const hasPhoto = isFilled.image(d.photo);
  const photoOnRight = d.image_position === true;
  ...
  const text = (
    <div className={styles.textCol}>
      ...
      <h2 className={styles.title}>{d.title || "The Pasha of Bonneval"}</h2>
      ...
      <div className={styles.body}>
        {isFilled.richText(d.body) ? (
          ...
          <p>
            Claude Alexandre, Comte de Bonneval — later Pasha of three tails in
            Constantinople — kept his finest Burgundy behind a library&apos;s
  ...
  return (
    ...
    >
      <div
        className={`${styles.inner} ${hasPhoto ? styles.withPhoto : ""} ${
          hasPhoto && photoOnRight ? styles.photoRight : ""
        }`}
      >
        {hasPhoto ? (
          photoOnRight ? (
